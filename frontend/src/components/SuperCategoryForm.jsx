...
const SuperCategoryForm = ({ maincategory ,triggerGettingSuperSubCategory,refreshCategories}) => {
  const { accessToken } = useContext(AuthContext);
  const [supercategories, setSuperCategories] = useState([{name: '' ,image:null }]);
  // const [fetchMainCategory,setFetchMainCategory] = useState([]);
  const [selectedCategory, setSelectedCategory]=useState('');

  // Function to add new input field
  const addCategoryField  = () => {
    setSuperCategories([...supercategories,  { name: "", image: null }]);
  };

    // Function to remove an input field
    const removeCategoryField  = (index) => {
      const values =[...supercategories];
      values.splice(index, 1);
      setSuperCategories(values);
    };
  ...
  const handleClearInputs = () => {
    const clearedList = supercategories.map((input) => ({ name:'', image: null }));
    setSuperCategories(clearedList);
  };
    ...
    <div className="container">
        ...
        <div className="card">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <button type="submit" className="btn btn-success">Save Super Category</button>
                </div>
